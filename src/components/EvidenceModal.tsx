...
export const EvidenceModal: React.FC<EvidenceModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-4xl w-full p-6 space-y-4 my-8">
        {/* Header */}
        <div className="flex items-start justify-between border-b border-slate-200 pb-3">
          <div className="space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-2.5 py-1 rounded-md text-xs font-bold font-mono bg-slate-900 text-white shadow-2xs">
              ...
              >
                {isResolved ? (
                  <>
                    ...
                    <span>PASSED (Discrepancy Resolved)</span>
                  </>
                ) : isPass ? (
                  <>
              ...
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-semibold bg-blue-50 text-blue-800 border border-blue-200">
                ...
                <span>
                  {matched} / {total} Parameters Matched ({matchPct}%)
                </span>
              </span>
          ...
          <div className="p-3 rounded-xl bg-amber-50/80 border border-amber-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
            <div className="space-y-0.5">
              <div className="flex items-center gap-1.5 font-bold text-amber-950">
                ...
                <span>Discrepancy Identified on Packet #{result.packetNumber || 1}</span>
              </div>
              <p className="text-amber-900 text-[11px]">
                {result.mismatchSummary || result.statusMessage}
              </p>
              <p className="text-slate-600 text-[10px]">
                For simple acceptable discrepancies (e.g. Origin format, minor manufacturer text variations), click to override.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              {onResolveDiscrepancy && (
                ...
                >
                  ...
                  <span>Ignore & Resolve Discrepancy (Mark as PASSED)</span>
                </button>
              )}
            </div>
          </div>
